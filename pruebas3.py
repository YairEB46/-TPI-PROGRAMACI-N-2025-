...
def registrar_boleta(cliente, productos_cliente, total):
    ...
    # Mostrar en GUI
    mostrar_boleta_ventana(cliente, contenido, boleta_nombre)


def mostrar_boleta_ventana(cliente, contenido, boleta_nombre):
    """Muestra la boleta en una ventana Tkinter."""
    ventana_boleta = tk.Tk()
    ventana_boleta.title(f"Boleta de Venta - {cliente}")
    ventana_boleta.geometry("500x400")
    ventana_boleta.config(bg="#858585")

    # Widget de texto
    texto_boleta = tk.Text(ventana_boleta, width=50, height=18, bg="lightyellow", 
    ...
    label = tk.Label(frame_info, text=f"Archivo: {boleta_nombre}", 
                     bg="#858585", fg="white", font=("Arial", 9))
    label.pack()

    # Botón para cerrar
    btn_cerrar = tk.Button(ventana_boleta, text="Cerrar", command=ventana_boleta.quit, 
